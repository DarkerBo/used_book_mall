    ...
    <ul class="goodsList">
      <li class="goodItem" v-for="item of goodsList" :key="item.id" @click="goDetail(item.id)">
        ...
        <div class="basic">
          ...
          <p class="price">
            <span class="now" style="margin-right: 40px;">&yen;{{item.price}}</span>
            <span
              class="amount"
              style="margin-right: 40px; color: #777;"
            >总销量{{item.sale_amount || 0}}件</span>
            <el-tag type="success" v-if="item.type === '1'">新品</el-tag>
            <el-tag type="danger" v-else-if="item.type === '2'">特价商品</el-tag>
          </p>
        </div>
        <span class="detail">点击进入商品详情&gt;&gt;</span>
      </li>
    </ul>
